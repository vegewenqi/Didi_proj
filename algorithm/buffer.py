...
class ReplayBufferWithAttention(object):
    ...
    def add(self, obs_ego_next, obs_others_next, done, ref_index, mask, weight):
        data = (obs_ego_next, obs_others_next, done, ref_index, mask)
        if self._next_idx >= len(self._storage):
            self._storage.append(data)
        else:
            self._storage[self._next_idx] = data
    ...
    def _encode_sample(self, idxes):
        obses_ego_next, obses_others_next, dones, ref_indexs, masks = [], [], [], [], []
        for i in idxes:
            data = self._storage[i]
        ...
        return np.array(obses_ego_next), np.array(obses_others_next), \
               np.array(dones), np.array(ref_indexs), np.array(masks)
