    ...
    def compute_rewards(self, obses, actions, untransformed_action, i=0.):
        ...
        with tf.name_scope('compute_reward') as scope:
            ...
            for ego_point in [ego_front_points, ego_rear_points]:
                ...
                veh2road4training += tf.where(logical_and(ego_point[1] > Road.U_Y1_D, dis_u_left < 1.0), tf.square(dis_u_left - 1.0), tf.zeros_like(veh_infos[:, 0]))
                veh2road4training += tf.where(logical_and(ego_point[1] > Road.U_Y2_D, dis_u_right > -1.0), tf.square(dis_u_right  - (-1.0)), tf.zeros_like(veh_infos[:, 0]))
            ...
            rewards = (0.01 * devi_v + 1.0 * devi_longitudinal + 1.0 * devi_lateral + 30 * devi_phi + 0.02 * punish_yaw_rate +
            ...
            reward_dict = dict(punish_steer0=punish_steer0,
                               ...
                               scaled_devi_longitudinal=1.0 * devi_longitudinal,
                               scaled_devi_lateral=1.0 * devi_lateral,
