    ...
    def compute_rewards(self, obses, actions, untransformed_action, i=0.):
        ...
        with tf.name_scope('compute_reward') as scope:
            ...
            for ego_point in [ego_front_points, ego_rear_points]:
                ...
                veh2road4training += tf.where(logical_and(right_flag, logical_and(ego_point[1] < Road.D_Y2_U, dis_d2_left < 1.0)), tf.square(dis_d2_left - 1.0), tf.zeros_like(veh_infos[:, 0]))
                veh2road4training += tf.where(logical_and(right_flag, logical_and(ego_point[1] < Road.D_Y3_U, dis_d2_right > 0.0)), tf.square(dis_d2_right - (0.0)) , tf.zeros_like(veh_infos[:, 0]))
                ...
                veh2road4training += tf.where(logical_and(straight_flag, logical_and(ego_point[1] > Road.U_Y1_D, dis_u_left < 1.0)), tf.square(dis_u_left - 1.0), tf.zeros_like(veh_infos[:, 0]))
                ...
                # end line for left
                veh2road4training += tf.where(logical_and(left_flag,logical_and(ego_point[0] < -Para.CROSSROAD_SIZE_LAT / 2 + Para.CROSSROAD_SIZE_LAT / 3, ego_point[1] > Para.OFFSET_L + Para.L_GREEN + Para.L_OUT_0 + Para.L_OUT_1 + Para.L_OUT_2 - 1.0)),
                    tf.square(ego_point[1] - (Para.OFFSET_L + Para.L_GREEN + Para.L_OUT_0 + Para.L_OUT_1 + Para.L_OUT_2 - 1.0)), tf.zeros_like(veh_infos[:, 0]))
                veh2road4training += tf.where(logical_and(left_flag,logical_and(ego_point[0] < -Para.CROSSROAD_SIZE_LAT / 2, ego_point[1] < Para.OFFSET_L + Para.L_GREEN + 1.0)),
                    tf.square(ego_point[1] - (Para.OFFSET_L + Para.L_GREEN + 1.0)), tf.zeros_like(veh_infos[:, 0]))

                # end line for right
                veh2road4training += tf.where(logical_and(right_flag, logical_and(ego_point[0] > Para.CROSSROAD_SIZE_LAT / 2 - Para.CROSSROAD_SIZE_LAT / 3, ego_point[1] > Para.OFFSET_R - 1.0)),
            ...
            rewards = (0.01 * devi_v + 0.8 * devi_longitudinal + 0.8 * devi_lateral + 30 * devi_phi + 0.02 * punish_yaw_rate +
                      ...
                      punish_a_x0 + punish_a_x1 + 0.05 * punish_a_x2) * tf.pow(Para.gamma, i)
